...
class PatternDelegateAdapter(val onPatternClick: (Pattern) -> Unit) : DelegateAdapter<ViewItem> {
  ...
  override fun onBindViewHolder(holder: RecyclerView.ViewHolder, item: ViewItem) {
    ...
    holder.bind(item.pattern, onPatternClick)
  }

  class PatternViewHolder(val view: View) : RecyclerView.ViewHolder(view) {
    fun bind(pattern: Pattern, onPatternClick: (Pattern) -> Unit) {
      view.patternTitle.text = pattern.name
      view.patternAuthor.text = "by ${pattern.designer.name}"
      view.previewImage.loadImg(pattern.firstPhoto.squareUrl)
